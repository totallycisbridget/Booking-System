...
def apply_theme_to_titlebar(root, theme_colors: Dict[str, str]):
    """Apply theme colors to the window title bar on Windows systems."""
...
def colors_from_tcl_namespace(
    root: Tk | None = None,
    namespace: str = "ttk::theme::sv_dark",
    array_name: str = "colors",
) -> Dict[str, str]:
...
def set_window_theme(
    root: Tk, style: Style, theme: str
) -> tuple[Style, Dict[str, str]]:
    ...
    theme_colors.setdefault(
        "-error", "#ff99a4"
    )  # Color inspected from sv_ttk entry error sprite
    theme_colors.setdefault(
        "-success", "#00c853"
    )  # Color inspected from sv_ttk entry success sprite
    match theme:
        case "dark":
            theme_colors.setdefault("-border", "#2f2f2f")
            theme_colors.setdefault("-buttonbg", "#2a2a2a")
        case "light":
            theme_colors.setdefault("-border", "#e7e7e7")
            theme_colors.setdefault("-buttonbg", "#fdfdfd")
...
def apply_all_theming(window: Tk, theme: str = "dark") -> tuple[Style, Dict[str, str]]:
    """Apply sv_ttk theming to the given window and configure custom styles."""
    # Create/get Style object for this window
    style = Style(master=window)

    # Set the base sv_ttk theme and get colors
    style, theme_colors = set_window_theme(window, style, theme)

    # Configure custom styles
    style.configure("Header.TLabel", font=("TkDefaultFont", 20, "bold"))
    style.configure("Large.TLabel", font=("TkDefaultFont", 16))
